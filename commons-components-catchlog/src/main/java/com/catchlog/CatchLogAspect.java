...
public class CatchLogAspect {
    ...
    private Object handleException(ProceedingJoinPoint joinPoint, Throwable e) {
        ...
        if (e instanceof BizException){
            ...
            return com.catchlog.ResponseHandler.handle(returnType, (BaseException)e);
        }

        log.error("UNKNOWN EXCEPTION :");
        log.error(e.getMessage(), e);

        return com.catchlog.ResponseHandler.handle(returnType, "UNKNOWN_ERROR", e.getMessage());
    }
    ...
    private void logRequest(ProceedingJoinPoint joinPoint) {
        try {
            ...
            for (Object arg : args) {
                log.debug("REQUEST : " + JsonUtils.toJson(arg));
            }
        }
        catch (Exception e){
            //swallow it
